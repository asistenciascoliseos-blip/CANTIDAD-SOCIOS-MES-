...
async function syncToCloud() {
  try {
    ...
    const mergedRecords = mergeRecords(cloudRecords, records);
    const response = await fetch(CLOUD_URL, {
      method: "POST",
      body: JSON.stringify({ action: "save", records: mergedRecords }),
    });
    const result = await response.json();
    if (!result.ok) throw new Error(result.error || "No se pudo guardar en Google Sheets.");
    records = mergedRecords.map(normalizeRecord);
    localStorage.setItem(STORAGE_KEY, JSON.stringify(records));
    render();
...
async function loadFromCloud() {
  try {
    ...
    localStorage.setItem(STORAGE_KEY, JSON.stringify(records));
    render();
    if (JSON.stringify(cloudRecords) !== JSON.stringify(mergedRecords)) await saveCloudRecords(mergedRecords);
...
async function fetchCloudRecords() {
  ...
  const result = await response.json();
  if (!result.ok || !Array.isArray(result.records)) throw new Error("No se pudieron leer los datos compartidos.");
  return result.records.map(normalizeRecord);
}

async function saveCloudRecords(recordsToSave) {
  const response = await fetch(CLOUD_URL, {
    method: "POST",
    body: JSON.stringify({ action: "save", records: recordsToSave }),
  });
  const result = await response.json();
  if (!result.ok) throw new Error(result.error || "No se pudieron guardar los datos compartidos.");
}
...
function mergeRecords(cloudRecords, localRecords) {
  ...
  [...cloudRecords, ...localRecords].forEach((record) => {
    if (record && record.date && record.shift) {
      merged.set(`${record.date}|${record.shift}`, normalizeRecord(record));
    }
  });
